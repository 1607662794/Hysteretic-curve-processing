...
# 原图
ax.plot(df['times(s)'], df['damage_index'] / 18.53644085626638, label='源数据',color=(58/255, 9/255, 100/255))
# plt.scatter(df['times(s)'], df['damage_index'], label='源数据', marker='o')#原绘图
# plt.scatter(df['cumulative deformation(mm)'], df['damage_index'], label='Original Data', marker='o', s=20)

# 筛选后的图
ax.scatter(filtered_df['times(s)'], filtered_df['damage_index'] / 18.53644085626638, label='采样数据', color=(58/255, 9/255, 100/255),s=30, marker='v',zorder=2)
# plt.scatter(filtered_df['times(s)'], filtered_df['damage_index'], label='过滤数据',c='orange', marker='s', edgecolors=['dimgray'])#专利绘图
...
# ax.title('Park-Ange损伤指标', font={'family': 'Times New Roman', 'size': 16})
# plt.xlabel('累计位移(mm)')
ax.set_xlabel('时间(s)', font={'family': 'SimSun', 'size': 22})
ax.set_ylabel('损伤比率', font={'family': 'SimSun', 'size': 22})
...
plt.savefig(f'E:\\研究生\\科研生活\\横向课题\\4.12-4.26冯老师教材\\绘图\\python绘图\\damage抽样图.png', dpi=600, bbox_inches='tight')
