...
if __name__ == '__main__':
    ...
    if show_predict:
        ...
        ax.set_ylabel('刚度退化比率', font={'family': 'SimSun', 'size': 28})
        degraded_stiff_array = np.array(degraded_stiff)
        s3 = plt.plot(times, degraded_stiff_array / 61.27174006657513,  label='真实值', zorder=1, color='red', linestyle='--')
        s4 = plt.scatter(test_times, test_degraded_stiff / 61.27174006657513, marker='s', s=50, color=(122/255, 27/255, 109/255), label='采样的真实值')
        s5 = plt.scatter(pre_x, pre_y / 61.27174006657513, color=(237/255, 104/255, 37/255), marker='^', s=50, label='采样的预测值')
        ...
        # 移动轴的边缘来为刻度标注腾出空间
        plt.tight_layout()
        plt.savefig(f'E:\\研究生\\科研生活\横向课题\\4.12-4.26冯老师教材\\绘图\\Python绘图\\测试集Stiffness退化预测.png', dpi=600,
                    bbox_inches='tight')
    ...
    if show_point_predict:  # 是否展示其中的某一个点
        ...
        # s6 = plt.scatter(pre_x[5], pre_y[5] / 61.27174006657513, c='mediumturquoise', marker='^', s=80)
        s6 = plt.scatter(pre_x[18], pre_y[18] / 61.27174006657513, c='mediumturquoise', marker='^', s=80)
        s8 = plt.plot(times, degraded_stiff_array / 61.27174006657513, label='Real labels', zorder=0, color='red',
                      linestyle='--')


        # plt.annotate('预测点', xy=(pre_x[0], pre_y[0] / 61.27174006657513), font={'family': 'SimSun', 'size': 25}, xytext=(pre_x[0] + 100, pre_y[0] / 61.27174006657513+0.15),
        #              arrowprops=dict(facecolor='0.2', shrink=0.05, headwidth=8, width=3), )
        # plt.annotate('预测点', xy=(pre_x[5], pre_y[5] / 61.27174006657513), font={'family': 'SimSun', 'size': 25}, xytext=(pre_x[0] + 25, pre_y[0] / 61.27174006657513+0.4),
        #              arrowprops=dict(facecolor='0.2', shrink=0.05, headwidth=8, width=3), )
        plt.annotate('预测点', xy=(pre_x[18], pre_y[18] / 61.27174006657513), font={'family': 'SimSun', 'size': 25}, xytext=(pre_x[0] + 100, pre_y[0] / 61.27174006657513+0.15),
                     arrowprops=dict(facecolor='0.2', shrink=0.05, headwidth=8, width=3), )
        ...
        # plt.savefig(f'E:\\研究生\\科研生活\\横向课题\\4.12-4.26冯老师教材\\绘图\\Python绘图\\point2_多任务学习单点预测Stiffness.png', dpi=600, bbox_inches='tight')
        # plt.savefig(f'E:\\研究生\\科研生活\\横向课题\\4.12-4.26冯老师教材\\绘图\\Python绘图\\point1_多任务学习单点预测Stiffness.png', dpi=600, bbox_inches='tight')
        plt.savefig(f'E:\\研究生\\科研生活\\横向课题\\4.12-4.26冯老师教材\\绘图\\Python绘图\\point3_多任务学习单点预测Stiffness.png', dpi=600, bbox_inches='tight')
