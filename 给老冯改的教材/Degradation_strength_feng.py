...
if __name__ == '__main__':
    ...
    if show_predict == False and show_point_predict == False:
        s1 = plt.scatter(times, degraded_strength, marker='s', s=10, edgecolors=['dimgray'])
    ...
    if period_print:
        ...
        # s2 = plt.scatter(cumulative_deformation2, Periodic_cycle_degraded_strength, marker='s', s=20,
        #                  edgecolors=['dimgray'])
        s2 = plt.scatter(times_2, Periodic_cycle_degraded_strength, marker='s', s=20,
                         edgecolors=['dimgray'])
    ...
    if show_predict:
        ...
        ax.set_ylabel('强度退化比率', font={'family': 'SimSun', 'size': 28})
        degraded_strength_array = np.array(degraded_strength)#只有数组可以在除以一个数之后全部都被除一遍
        s3 = plt.plot(times, degraded_strength_array / 53.02, label='真实值', zorder=1, color='red', linestyle='--')
        s4 = plt.scatter(test_times, test_degraded_strength / 53.02, marker='s', color=(122/255, 27/255, 109/255), s=50, label='采样点真实值')
        s5 = plt.scatter(pre_x, pre_y / 53.02, color=(237/255, 104/255, 37/255), marker='^', s=50, label='采样点预测值')
        ...
        # 移动轴的边缘来为刻度标注腾出空间
        plt.tight_layout()
        plt.savefig(f'E:\\研究生\\科研生活\\横向课题\\4.12-4.26冯老师教材\\绘图\Python绘图\\训练集strength退化预测.png', dpi=600,
                    bbox_inches='tight')

    if show_point_predict:  # 是否展示其中的某一个点
        fig, ax = plt.subplots()
        # plt.title('Bearing capacity degradation index', font={'family': 'Times New Roman', 'size': 20})
        ax.set_xlabel('时间(s)', font={'family': 'SimSun', 'size': 28})
        ax.set_ylabel('强度退化比率', font={'family': 'SimSun', 'size': 28})
        degraded_strength_array = np.array(degraded_strength)  # 只有数组可以在除以一个数之后全部都被除一遍
        s7 = plt.scatter(pic_index_times, pic_index_degraded_strength / 53.02, color=(122/255, 27/255, 109/255), marker='s')
        # s6 = plt.scatter(pre_x[0], pre_y[0] / 53.02, c='mediumturquoise', marker='^', s=80)
        s6 = plt.scatter(pre_x[5], pre_y[5] / 53.02, c='mediumturquoise', marker='^', s=80)
        # s6 = plt.scatter(pre_x[18], pre_y[18] / 53.02, c='mediumturquoise', marker='^', s=80)
        s8 = plt.plot(times, degraded_strength_array / 53.02, label='Real labels', zorder=0, color='red',
                      linestyle='--')
        # plt.annotate('预测点', xy=(pre_x[0], pre_y[0] / 53.02), font={'family': 'SimSun', 'size': 25}, xytext=(pre_x[0] - 1000, pre_y[0] / 53.02-0.1),
        #              arrowprops=dict(facecolor='0.2', shrink=0.05, headwidth=8, width=3), )
        plt.annotate('预测点', xy=(pre_x[5], pre_y[5] / 53.02), font={'family': 'SimSun', 'size': 25}, xytext=(pre_x[0] - 1400, pre_y[0] / 53.02-0.03),
                     arrowprops=dict(facecolor='0.2', shrink=0.05, headwidth=8, width=3), )
        # plt.annotate('预测点', xy=(pre_x[18], pre_y[18] / 53.02), font={'family': 'SimSun', 'size': 25}, xytext=(pre_x[0] - 500, pre_y[0] / 53.02-0.1),
        #              arrowprops=dict(facecolor='0.2', shrink=0.05, headwidth=8, width=3), )

        # plt.legend((s1, s6), ('true label', 'predict label'), loc='best')
        plt.legend((s7, s6), ('真实值', '预测值'), loc='best', prop={'family': 'SimSun', 'size': 25})

        # 刻度值字体大小设置（x轴和y轴同时设置）
        plt.tick_params(labelsize=20)
        # plt.savefig(f'E:\\研究生\\科研生活\\横向课题\\4.12-4.26冯老师教材\绘图\\Python绘图\\point2_多任务学习单点预测Srength.png', dpi=600,bbox_inches='tight')
        plt.savefig(f'E:\\研究生\\科研生活\\横向课题\\4.12-4.26冯老师教材\绘图\\Python绘图\\point1_多任务学习单点预测Srength.png', dpi=600,bbox_inches='tight')
        # plt.savefig(f'E:\\研究生\\科研生活\\横向课题\\4.12-4.26冯老师教材\绘图\\Python绘图\\point3_多任务学习单点预测Srength.png', dpi=600,bbox_inches='tight')
    ...
    if save_dir:
        label = pd.DataFrame(
            {'image_names': image_names, 'cumulative deformation(mm)': cumulative_deformation,
             'degraded strength(KN)': degraded_strength, 'times(s)': times})
