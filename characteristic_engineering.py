...
np.savetxt(r"E:\研究生\科研生活\滞回曲线实操\数据库-new\Shear_compression_tests\RS1\characteristic_curve.csv", result,
           delimiter=",", fmt='%.4f',
...
# plt.subplot(1, 2, 2)
if True:
    '''注意这个耗能没有实际意义，只是将横纵坐标增量相乘了'''
    plt.plot(cumulative_deformation, cumulative_energy)
    plt.scatter(cumulative_deformation, cumulative_energy, marker='s', edgecolors=['dimgray'])
    # plt.title("cumulative_energy")
    plt.title("累计耗能")
    plt.rcParams['font.sans-serif'] = ['SimHei']  # 显示中文标签
    plt.rcParams['axes.unicode_minus'] = False
    # plt.xlabel("cumulative deformation(mm)")
    # plt.ylabel("cumulative energy(kN·mm)"
    plt.xlabel("累计位移(mm)")
    plt.ylabel("累计耗能(kN·mm)")
plt.rcParams['font.sans-serif'] = ['SimHei']  # 显示中文标签
plt.rcParams['axes.unicode_minus'] = False
