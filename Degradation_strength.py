...
if __name__ == '__main__':
    ...
    if show_predict:
        ...
        s3 = plt.plot(times, degraded_strength_array / 53.02, label='Real labels', zorder=1, color='red', linestyle='--')
        s4 = plt.scatter(test_times, test_degraded_strength / 53.02, marker='s', color=(122/255, 27/255, 109/255), s=50, label='Real labels of samples')
        s5 = plt.scatter(pre_x, pre_y / 53.02, color=(237/255, 104/255, 37/255), marker='^', s=50, label='Predicted labels of samples')
        # plt.legend((s4, s5), ('true label', 'predict label'), loc='best')

        plt.legend(loc='best', prop={'family': 'Times New Roman', 'size': 17.5})
        ...
        # 移动轴的边缘来为刻度标注腾出空间
        plt.tight_layout()
        plt.savefig(f'E:\Code\Image regression\论文绘图\python绘图保存\测试集strength退化预测.png', dpi=600,
                    bbox_inches='tight')
    ...
    if show_point_predict:  # 是否展示其中的某一个点
        ...
        plt.annotate('Prediction points', xy=(pre_x[0], pre_y[0] / 53.02), font={'family': 'Times New Roman', 'size': 20}, xytext=(pre_x[0] - 1400, pre_y[0] / 53.02-0.1),
        ...
        plt.savefig(f'E:\Code\Image regression\论文绘图\python绘图保存\point2_多任务学习单点预测Srength.png', dpi=600,
                    bbox_inches='tight')
